...
import { 
  ...
  TrendingUp, Shield, ExternalLink, CheckCircle2, Brain
} from 'lucide-react';
...
export const StructuredAnalysis: React.FC<StructuredAnalysisProps> = ({ data }) => {
  ...
  return (
    <div className="w-full space-y-3 text-left">
      ...
      <div className="bg-gradient-to-r from-cyan-900/40 to-purple-900/40 border border-cyan-700/50 rounded-2xl p-4">
        <div className="flex items-center gap-2 mb-2">
          <span className="bg-cyan-500 text-slate-950 text-[9px] font-black px-2 py-0.5 rounded-full uppercase">
            {data.tipo?.replace('_', ' ') || 'Análise'}
          </span>
          {data.consultado_em && (
            <span className="text-[10px] text-slate-400">
              Atualizado: {new Date(data.consultado_em).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
            </span>
          )}
        </div>
        <h3 className="text-base font-black text-white mb-1">{data.titulo}</h3>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="flex items-center gap-2 text-slate-300">
              ...
              <span>{data.partida.competicao || 'Não informado'}</span>
            </div>
            <div className="flex items-center gap-2 text-slate-300">
      ...
      {data.analise_tatica && (
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4">
          <h4 className="text-[10px] font-black text-purple-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            <Brain className="w-3 h-3" /> Análise Tática
          </h4>
          ...
          <div className="space-y-3">
            {data.mercados.map((m, i) => (
              <div key={i} className="bg-slate-950/50 border border-slate-800 rounded-xl p-3">
                ...
                {m.argumentos && m.argumentos.length > 0 && (
      ...
      {data.conclusao && (
        <div className="bg-gradient-to-r from-cyan-950/30 to-slate-900/50 border border-cyan-800/50 rounded-2xl p-4">
          <h4 className="text-[10px] font-black text-cyan-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
            🎯 Conclusão
          </h4>
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            {data.fontes.map((f, i) => (
              <a 
                ...
                target="_blank" 
                rel="noreferrer" 
                className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-slate-300 px-2 py-1 rounded-md transition"
              >
